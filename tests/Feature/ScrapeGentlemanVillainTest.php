...
class ScrapeGentlemanVillainTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function podcast_can_be_scraped()
    {
        Http::fake(
            CreateMegaphoneResponse::init()
                ->setUrl('https://player.megaphone.fm/playlist/PHL7808404854')
                ->addEpisode()
                ->generate()
        );

        $this->artisan('scrape regal');

        $episode = Episode::first();
        $this->assertEquals(1, Episode::count());
        $this->assertEquals('megaphone.fm', $episode->source);
        $this->assertEquals(1, $episode->source_id);
        $this->assertEquals('Gentleman Villain', $episode->program);
