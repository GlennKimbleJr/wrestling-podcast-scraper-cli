...
class ScrapeFoleyIsPodTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function podcast_can_be_scraped()
    {
        Http::fake(
            CreateMegaphoneResponse::init()
                ->setUrl('https://player.megaphone.fm/playlist/PHL4244076691')
                ->addEpisode()
                ->generate()
        );

        $this->artisan('scrape foley');

        $episode = Episode::first();
        $this->assertEquals(1, Episode::count());
        $this->assertEquals('megaphone.fm', $episode->source);
        $this->assertEquals(1, $episode->source_id);
        $this->assertEquals('Foley Is Pod', $episode->program);
