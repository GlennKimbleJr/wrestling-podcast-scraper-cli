...
class ScrapeDDPSnakePitTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function podcast_can_be_scraped()
    {
        Http::fake(
            CreateMegaphoneResponse::init()
                ->setUrl('https://player.megaphone.fm/playlist/PHL1466141192')
                ->addEpisode()
                ->generate()
        );

        $this->artisan('scrape ddp');

        $episode = Episode::first();
        $this->assertEquals(1, Episode::count());
        $this->assertEquals('megaphone.fm', $episode->source);
        $this->assertEquals(1, $episode->source_id);
        $this->assertEquals('DDP Snake Pit', $episode->program);
